...
//-----------------------------------------------------------------------------
  void DaqanaFilter::print_(const std::string& Message, const std::source_location& location) {
...
//-----------------------------------------------------------------------------    
    ...
    if (!ArtEvent.getByLabel(_tcCollTag, tcch)) {
      TLOG(TLVL_ERROR) << "No time cluster collection tag:" << _tcCollTag.data();
      _tcc = nullptr;
    }
    else {
      _tcc   =  tcch.product();
