...
//-----------------------------------------------------------------------------
int fit_dr_vs_straw(const char* DsID, int RunNumber,int Panel) {
  ...
  TH2F* h0 = (TH2F*) f->Get(Form("//digis/ssht_%03i/dr_vs_straw",800+Panel));

  int nx = 96; // h0->GetNbinsX();
...
//-----------------------------------------------------------------------------
  ...
  for (int i=0; i<nx; i++) {
    ...
    double edt  = fr[i].e[1];
    double sig  = fr[i].p[2];
    double esig = fr[i].e[1];
    
    printf(" %10.5f %12.5f  %12.5f %12.5f  %12.5f\n",
