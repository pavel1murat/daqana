...
public:
  ...
  PlotN001(int RunNumber, int RecoVersion=0, const char* Fn = "");
  PlotN001(const char* Fn);
  ~PlotN001();
...
//-----------------------------------------------------------------------------
PlotN001::PlotN001(int RunNumber, int RecoVersion, const char* Fn) {
  ...
  if (fn == "") {
    ...
    std::string dsid    = std::format("nts.mu2e.trk.vst00s000r000n001.root");
    fn                  = std::format("{}/{}/nts.mu2e.trk.vst00s000r000n001.{:06d}_000001.root",
  ...
  if (fFile == nullptr) {
    std::cout << std::format("ERROR: coudn't open the file:{}\n",fn);
  }
  else {
...
//-----------------------------------------------------------------------------
PlotN001::PlotN001(const char* Fn) {
...
//-----------------------------------------------------------------------------
int PlotN001::plot_panel_edep(int Panel) {
...
//-----------------------------------------------------------------------------
int PlotN001::plot_panel_dt(int Panel1, int Panel2) {
...
//-----------------------------------------------------------------------------
int PlotN001::plot_plane_dt(int Station) {
  std::string hname(Form("h_plane_dt_%06i",fRunNumber));
  
  std::string var  = Form("sd.tdc0-tc.timep(1)");
