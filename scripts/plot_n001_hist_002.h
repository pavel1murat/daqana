...
// 1. plot delta(t) between the panels in the same plane
//////////////////////////////////////////////////////////
...
public :
  ...
  struct PanelData_t {
    TrkPanelMap_t::Data* tpm;
                                        // SD index in the event
    std::vector<int>     index[96];
  };
  
  struct PlaneData_t {
    int plane;
    PanelData_t* panel_data[6];
  }
    
  struct Data_t {
    std::vector<PlaneData_t*> plane_data;
  };
...
// Fixed size dimensions of array or collections stored in the TTree if any.
 ...
 //DaqEvent        *evt;
  ...
  virtual void     Loop(int Mnid1, int Ch1, int Mnid2, int Ch2, int NEvents, double DtMin = 0, double DtMax = -1);
  virtual bool     Notify();
