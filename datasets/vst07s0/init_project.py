...
#-------v----------------------------------------------------------------------
        self.add_dataset(Dataset('raw.mu2e.trk.vst.art'               ,'vst07s0s00r0000','local'))
        self.add_dataset(Dataset('rec.mu2e.vst07s0s10r0000.daqana.art','vst07s0s10r0000','local'))
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
    ...
    def __init__(self,idsid=None):
        ...
        ProjectBase.__init__(self,project='daqana',family_id='vst07s0',idsid=idsid);
        self.init_datasets();
#------------------------------------------------------------------------------
# stage 1
# -------
# s1:cosmic_reco : InputDsID is 'vst07s0s00r0000'
#                  reconstruction job has only one output stream
#------------------------------------------------------------------------------        
        s                            = self.new_stage('s1');
        input_dsid                   = idsid;
        if (input_dsid == None): input_dsid = 'vst07s0s00r0000'
...
#------------------------------------------------------------------------------        
        if (input_dsid == None): input_dsid = 'vst07s0s10r0000'
