...
#-------v----------------------------------------------------------------------
        self.add_dataset(Dataset('raw.mu2e.trk.vst.art'               ,'vst05s0s00r0000','local'))
        self.add_dataset(Dataset('rec.mu2e.vst05s0s10r0000.daqana.art','vst05s0s10r0000','local'))
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------
    ...
    def __init__(self,idsid=None):
        ...
        ProjectBase.__init__(self,project='daqana',family_id='vst05s0',idsid=idsid);
        self.init_datasets();
#------------------------------------------------------------------------------
# stage 1
# -------
# s1:cosmic_reco : InputDsID is 'vst05s0s00r0000'
#                  reconstruction job has only one output stream
#------------------------------------------------------------------------------        
        s                            = self.new_stage('s1');
        input_dsid                   = idsid;
        if (input_dsid == None): input_dsid = 'vst05s0s00r0000'
